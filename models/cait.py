""" 
adapted from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/cait.py
...
class Cait(nn.Module):
    ...
    def forward(self, pixel_values, output_attentions=False):
        x, attentions = self.forward_features(pixel_values)
        x = self.head(x)
...
class CaiTWrapper(nn.Module):
    ...
    def forward(self, pixel_values, output_attentions=False):
        out = self.backbone(pixel_values, output_attentions)
        if output_attentions:
            out, attentions = out
        else:
            attentions = None
        out = self.classifier(out)
        return SequenceClassifierOutput(logits=out, attentions=attentions)
